...
import { addLink, getLink, getAllLinks, deleteExpiredLinks } from './lib/link-db.mjs';
...
try {
  ...
  // Test 3: Add link with future expiration
  console.log('\n3. Adding link with future expiration...');
  ...
  // Test 4: Add link with past expiration (manually for testing)
  console.log('\n4. Adding link with past expiration (for testing)...');
  ...
  allLinks.forEach(link => {
    console.log(`   - ${link.id}: ${link.url} (expired: ${link.isExpired})`);
  });

  // Test 6: Delete expired links
  console.log('\n6. Deleting expired links...');
