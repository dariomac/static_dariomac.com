...
const hbHelpers = {
  ...
  'ifIsOrWasInProgress': function(columnId, options) {
    if (columnId === 'progress_2' || columnId === 'done_3' || columnId === 'archive_6') {
      return options.fn(this);
    }
    return options.inverse(this);
  },
  'ifIsOrWasDone': function(columnId, options) {
    if (columnId === 'done_3' || columnId === 'archive_6') {
      return options.fn(this);
    }
    return options.inverse(this);
  },
  'ifDone': function(columnId, options) {
    if (columnId === 'done_3') {
      return options.fn(this);
    }
    return options.inverse(this);
  ...
  // Normalizes loosely-formatted date strings (e.g. "Mar 19, 2026", "20260223")
  // into ISO 8601 (YYYY-MM-DD) for use in JSON-LD/sitemap fields. Accepts any
  // number of candidate values and uses the first one that parses.
  'isoDate': (...args) => {
    ...
    for (const value of args) {
      ...
      const parsed = moment(value.trim(), formats, true);
      if (parsed.isValid()) {
        return parsed.format('YYYY-MM-DD');
      }
    }

    return args.find((value) => typeof value === 'string' && value.trim()) || '';
  },
};
