...
const hbHelpers = {
  ...
  'highlighter': (text) => {
    ...
    const highlightWords = [
      '\\.net',
      'architect','asp','angular',
      'blog posts',
      'c#','cms','coo','conferences as speaker',
      'decision making',
      'embedded visual basic 3.0','extremme programing',
      'devops','drupal',
      'javascript','java','jsps','jsp','jquery',
      'hp-jornada 720','hp ipaq-2210','highcharts js',
      'quality assurance',
      'linux',
      'nginx','nodejs',
      'oracle',
      ...
      'scrum master','scrum','scientific research','sdlc','social networks','sql server','sql','svg',
      'tfs',
      'visual basic 6.0',
      'windows'
    ];
...
function convertCollectionFiles () {
  for (var i = 0; i < exports.collectionPages.length; i++) {
    ...
    _.each(_.keys(exports.collectionJson), function (laneid) {
      ...
      exports.collectionJson[laneid] = exports.collectionJson[laneid].sort((a, b) => {
        ...
        if (aPos && aPos !== NaN) {
          ...
          return 1; // I asume that if b is NaN, then it'll be always on the bottom
        }
        return 0; // If both are NaN, they are equals
      });
