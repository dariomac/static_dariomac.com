...
const settings = [
  ...
  function fillObjLayoutAndSave (dmdObj) {
    ...
    let outputFilePath = `${exports.paths.outputPath}${dmdObj.meta.outputFilename}`;
  ...
  try {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      switch (type) {
        ...
        let renderer = htmlrenderer; //new commonmark.HtmlRenderer();
  ...
  if (entering) {
    if (this.disableTags === 0) {
      if (this.options.safe && potentiallyUnsafe(node.destination)) {
        ...
        const iName = path.basename(node.destination, path.extname(node.destination));
