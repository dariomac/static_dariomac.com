...
export function generateRedirectPage(destinationUrl, slug, ogTags = {}, debug = false) {
  // Generate Open Graph meta tags
  const destinationMetaTags = generateMetaTags(ogTags, destinationUrl);

  const escapedDestinationURL = escapeHtml(destinationUrl);
  const escapedTitle = escapeHtml(ogTags['og:title']);

  return `<!DOCTYPE html>
<html lang="en">
<head>
  <title>${escapedTitle || 'Redirecting...'}</title>
  
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="robots" content="noindex, nofollow">
  
  <!-- Meta tags for social media preview / SEO -->
  ${destinationMetaTags}

  ${debug ? '' : `<!-- Google tag (gtag.js) -->
  ...
  <style>
    body {
      ...
      height: 100vh;
      margin: 0;
      background: linear-gradient(135deg, #FF6E19 0%, #764ba2 100%);
      color: white;
    }
    .container {
      text-align: center;
  ...
  ${debug ? '' : `<meta http-equiv="refresh" content="2;url=${escapedDestinationURL}">`}
</head>
<body>
  <div class="container">
    <div class="spinner"></div>
    <h1>Redirecting...</h1>
    <p>If you're not redirected, <a href="${escapedDestinationURL}" id="manual-link">click here</a></p>
  </div>

  <script>
    // Destination URL
    const destination = '${escapedDestinationURL}';
    
    ${debug ? '' : `
    // Track the redirect event in Google Analytics
    gtag('event', 'page_view', {
      page_path: '/go/${escapeJs(slug)}',
      page_title: '${escapedTitle || 'Undefined'}',
 ...
 */
function generateMetaTags(ogTags, destinationUrl) {
  const tags = [];
  ...
  /// Add Favicon link
  // Get favicon URL
  const faviconUrl = ogTags['favicon'] || `${new URL(destinationUrl).origin}/favicon.ico`;
  tags.push(`  <link rel="icon" href="${escapeHtml(faviconUrl)}"/>`);
