...
function parseOGTags(html, baseUrl) {
  ...
  while ((match = metaRegex.exec(html)) !== null) {
    ...
    if (property.startsWith('og:') || property.startsWith('twitter:') || property === 'description') {
      ...
      // Resolve relative URLs for image, audio, video, and url properties
      if (
        property.includes(':image') || 
        property.includes(':audio') || 
        property.includes(':video') || 
        property === 'og:url' ||
        property === 'twitter:player'
      ) {
         decodedContent = resolveUrl(decodedContent, baseUrl);
      }

      ogTags[property] = decodedContent;
    }
  }
...
function extractFavicon(html, baseUrl) {
  ...
  for (const pattern of faviconPatterns) {
    ...
    if (match && match[1]) {
      ...
      // Check if the captured URL has a valid file extension
      // This prevents matching GitHub's data-base-href or other incomplete URLs
      const hasExtension = /\.(ico|png|jpg|jpeg|gif|svg|webp)(\?|$)/i.test(faviconPath);

      if (hasExtension) {
        return resolveUrl(faviconPath, baseUrl);
      }
      // If no extension, continue to try other patterns
    }
  }
