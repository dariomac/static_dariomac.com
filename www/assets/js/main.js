jQuery(document).ready(function($){
	...
	*************************************/
	...
	/*$('.dm-board ul').mixItUp({
	    ...
	    callbacks: {
	    	onMixStart: function(){
	    		$('.dm-fail-message').fadeOut(200);
	    	},
	      	onMixFail: function(){
	      		$('.dm-fail-message').fadeIn(200);
	    	}
	    }
	});*/
	...
	/*$(".dm-filter-content input[type='search']").keyup(function(){
	  	// Delay function invoked to make sure user stopped typing
	  	delay(function(){
	    	inputText = $(".dm-filter-content input[type='search']").val().toLowerCase();
	    	...
	    	if ((inputText.length) > 0) {
	      		...
	      		$('.dm-board ul').mixItUp('filter', $matching);
	    	} else {
	      		// resets the filter to show all item if input is empty
	      		$('.dm-board ul').mixItUp('filter', 'all');
	    	}
	  	}, 200 );
	});*/
});
...
var buttonFilter = {
  	...
  	init: function(){
    	...
    	self.$filters = $('.dm-main-content');
    	self.$container = $('.dm-board ul');

	    self.$filters.find('.dm-filters').each(function(){
	      	var $this = $(this);
